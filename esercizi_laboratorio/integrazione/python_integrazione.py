...
for numeroPunti in OrdiniDivisione:
    ...
    puntiGriglia = np.linspace( estremoInferiore, estremoSuperiore, numeroPunti ); #serie di N-1 intervalli equispaziati
    step = puntiGriglia[1]-puntiGriglia[0]
    
    
    if len(puntiGriglia) % 2 != 0:
        print("Numero di punti totali è dispari: simpson NON funziona")
    ...
    for index in range( 1, len(puntiGriglia)-1 ): #non include l'ultimo punto e il primo delle ascisse
        ...
        if index % 2 == 0: #punti di indice pari
            sommaSimpson += ( (2/3) * np.e ** puntiGriglia[index] )
        
        else: #restanti punti di indice dispari
            sommaSimpson += ( (4/3) * np.e ** puntiGriglia[index] )
        
    
    #punti di bordo
    sommaSimpson += ( (1/3) * np.e ** puntiGriglia[0] )
